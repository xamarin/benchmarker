...
class ProcessBinaryProtocolFiles(MasterShellCommand):
    ...
    def start(self):
        ...
        if runids is None or runids == []:
            ...
            self.command = ['timeout', '--foreground', '--signal=15', '360', 'bash', '-x', '-c', 'failed=0; ' + "".join([compare_cmd(log_full_path, runid) for (log_full_path, runid) in zip(logs_full_path, runids)]) + ' if [ "$failed" == "1" ]; then exit 1; fi ']
